...
RSpec.describe Spree::Api::V3::Admin::StoreController, type: :controller do
  ...
  # Scoped to just #show/#update (not the whole describe block): this
  # context's `before` stubs `current_store` directly via
  # `allow_any_instance_of(Spree::Api::V3::BaseController).to
  # receive(:current_store)`, which would silently swallow the header
  # resolution logic under test further down in this file.
  context 'with a single current store' do
    include_context 'API v3 Admin authenticated'

    # `@default_store` is a shared in-memory object across the suite; reload
    # it so each example starts from the persisted state and isn't tripped up
    # by AR change tracking from earlier rolled-back updates.
    before { store.reload }
