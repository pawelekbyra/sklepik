...
# incident: the repo's initial commit predates the Vercel project, so nothing
# ever auto-deploys without an explicit trigger, and a build that errors must
# fail the run immediately rather than exhaust the 5-minute poll. A separate
# top-level describe so this doesn't inherit the stubs from the block above
# (an outer `before` re-stubbing methods on `service` would apply here too if
# nested, since `service` still resolves via subject-override).
RSpec.describe Spree::Provisioning::ProvisionStore, 'deployment triggering and polling' do
