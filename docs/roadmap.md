...
**Decyzja 2026-07-13, ODWRÓCONA 2026-07-17:** model docelowy dla Fazy 3 (niezależne sklepy) **nie jest już** "Store Factory" (repo + Vercel per sklep). Właściciel odrzucił ten kierunek po dziewięciu research-passach nad branżowymi wzorcami multi-tenant — nowy model docelowy to jeden współdzielony storefront z layoutem jako danymi: [`docs/plans/storefront-composition-system.md`](plans/storefront-composition-system.md) (aktywny). [`docs/plans/store-factory.md`](plans/store-factory.md) zachowany jako materiał historyczny, superseded. Konta klientów: **osobne per sklep** (decyzja właściciela, nadal aktualna, niezależna od tej zmiany) — osobny plan [`docs/plans/per-store-customer-accounts.md`](plans/per-store-customer-accounts.md).

**Etap 0 Store Factory (fundament wielosklepowości backendu) — WDROŻONY NA PRODUKCJĘ, POZOSTAJE AKTUALNY:** produkcja ma host resolution, cache per sklep, atomowy `StoresController#create`, backendowy EUR sync i naprawione role sklepu. Bieżący zestaw dodaje globalne rozwiązanie aktywnego publishable API key → store przed autoryzacją, odrzuca sprzeczny host/`X-Spree-Store-Id`, rozszerza tenantowy kontekst cache i naprawia pusty 422. Lokalna regresja: 50 przykładów RSpec, 0 failures. **Ważne:** ten etap jest fundamentem wielosklepowości niezależnie od modelu Store Factory vs composition-system powyżej — pozostaje w pełni aktualny i wymagany. **Pozostaje:** pełny integracyjny scenariusz dwóch realnych tenantów obejmujący katalog, koszyk, klientów i zamówienia oraz `rswag:specs:swaggerize` dla `/admin/stores`.

**Etap 1 Store Factory (kontrakty SDK) — WDROŻONY, POZOSTAJE UŻYTECZNY:** typy kontraktów są eksportowane z `@spree/sdk`; `@sklepik/test-contracts` jest w frozen lockfile i używa rzeczywistego API SDK. Typecheck, build i 5/5 testów jednostkowych przechodzą. Te typy/testy izolacji tenantów pozostają przydatne w nowym modelu (composition-system) — nie są specyficzne dla repo-per-sklep.

**Etap 2+ Store Factory (ręczny pilot, automatyczny provisioning repo/Vercel per sklep) — PORZUCONY 2026-07-17.** Nie kontynuować. Następne kroki wielosklepowości: patrz Migration Path w `storefront-composition-system.md` (middleware tenanta, `SklepikPageRepository`, montaż silnika edytora z `edytor-sklepu`). Stripe, strony prawne i checkout E2E nadal blokują start sprzedaży niezależnie od tej zmiany.
