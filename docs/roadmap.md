...
Naprawiono pięć usterek w testach zaraz po zmergowaniu F15-F24:
...
4. **Zawieszony Dashboard E2E (2,5h) — regresja z F21 frontend**: trzy nowe pliki tras (`settings/shipping-methods.tsx`, `tax-rates.tsx`, `zones.tsx`) miały `createFileRoute('...' as any)`. Rzutowanie `as any` (dodane, żeby ominąć błąd TS wynikający z braku tras w `routeTree.gen.ts`) sprawia, że argument nie jest literałem stringa, więc generator TanStack Router **wywala się** przy transformacji tych plików (`expected route id to be a string literal`). Efekt: dev-server Vite nie generuje drzewa tras → panel się nie ładuje → wszystkie 153 testy E2E timeoutują (dot-reporter pokazywał `××T××T…` przez 2,5h, aż concurrency anulował run). Poprawka: usunięto `as any` z trzech plików i przegenerowano `routeTree.gen.ts` (dodane 3 trasy, diff czysto addytywny). `as any` był hackiem obchodzącym objaw zamiast przyczyny — właściwie trasy muszą być w wygenerowanym drzewie.
5. **Rate limiting (F16) dławił E2E — druga przyczyna zawieszki E2E**: po naprawie tras (#4) build/typecheck panelu przechodził, ale E2E dalej wisiał ~2,5h. Przyczyna: helper `login(page)` (e2e/helpers.ts) robi realny POST na `/api/v3/admin/auth/login` i jest wołany przez ~każdy z 153 testów, wszystkie z jednego loopback IP. Reguła rack_attack z F16 (`admin/auth/login/ip`, 10/IP/godz.) po 10 logowaniach zwracała 429 na każde kolejne → strona nie wychodziła z `/login` → `expect(page).not.toHaveURL(/\/login/)` timeoutował (15s) → ~143 testy w timeout = godziny. Poprawka: `Rack::Attack.enabled = false if Rails.env.test?` w initializerze — rate limiting to zabezpieczenie produkcyjne, w środowisku testowym nie może dławić suite (żaden spec nie asertuje throttlingu). Zweryfikowane w źródle gemu: middleware robi `return @app.call(env) if !enabled`, czyli pełny pass-through.
