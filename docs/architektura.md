...
- **Operator zarządza:** dashboard → Admin API (JWT po zalogowaniu; single-origin proxy `/api/*` → Oracle Cloud w `packages/dashboard/vercel.json`, zmienione z Rendera 2026-07-09), dzięki czemu httpOnly refresh cookie działa bez CORS. **Uwaga dla agentów (2026-07-11):** ten proxy jest celową decyzją architektoniczną, nie prowizorką do zastąpienia bezpośrednimi wywołaniami (`VITE_SPREE_API_URL`) — backend nie ma skonfigurowanego CORS dla originu dashboardu, więc bezpośrednie wywołania zostaną zablokowane w przeglądarce, a nawet gdyby CORS dodać, refresh-cookie pod `SameSite=None` cross-origin jest bardziej kruchy niż same-origin. Jeśli login zwraca 500/CORS error, zdiagnozuj najpierw `curl` bezpośrednio do `https://141-253-103-172.nip.io/...` z pominięciem Vercela — to od razu pokazuje, czy problem jest w backendzie (najczęściej: postgres/redis padły, zobacz sekcję niżej) czy w samym proxy.
