...
- **Następny krok dla kogokolwiek to kontynuuje (lokalnie):** uruchomić skrypt raz na jednym testowym sklepie, poprawić niezweryfikowane fragmenty (git-link projektu, trigger deploya) na podstawie rzeczywistych odpowiedzi API, potem dopiero przechodzić do Etapu 3 (`ProvisioningRun`/Sidekiq w backendzie, GitHub App zamiast osobistego PAT) i wpięcia w formularz `/$storeId/new-store` w panelu.
- **Cienki starter (`sklepik-store-template`) nadal nie istnieje** — świadomy skrót na ten pilot: kopiujemy cały `sklepikFront` jako pierwszy template, refaktor do wersjonowanych pakietów `@sklepik/*` zostaje na później (zgodnie z pierwotnym planem — nie budować z wyprzedzeniem).
